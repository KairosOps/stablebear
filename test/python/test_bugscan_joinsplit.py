...
def test_valid_split_unaffected():
    parts = sb.split(sb.FloatTensor(np.arange(6.0)), 3)
    assert [tuple(p.shape) for p in parts] == [(2,), (2,), (2,)]
    assert np.asarray(parts[0]).tolist() == [0.0, 1.0]
