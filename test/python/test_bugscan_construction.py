...
def test_scalar_ndarray_zero_round_trips():
    """The zero value must round-trip too (it previously matched only by luck)."""
    assert np.asarray(sb.FloatTensor(np.array(0.0))).item() == 0.0
    assert np.asarray(sb.IntTensor(np.array(0))).item() == 0
