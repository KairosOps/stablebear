...
def test_numpy_parity():
    """NumPy raises on the identical operations (read-only broadcast view)."""
    nb = np.broadcast_to(np.array([1.0, 2.0, 3.0]), (2, 3))
    with pytest.raises(ValueError):
        nb[0, 0] = 9.0
    with pytest.raises(ValueError):
        nb += 1.0
...
def test_contiguous_tensor_still_writeable():
    t = sb.FloatTensor(np.array([1.0, 2.0, 3.0]))
    t[0] = 9.0
    t += 1.0
    assert np.asarray(t).tolist() == [10.0, 3.0, 4.0]
