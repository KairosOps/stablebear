...
* **Sampling functions reject degenerate inputs instead of segfaulting** — `point_process.sample_poisson` crashed the interpreter (SIGSEGV) whenever the Poisson mean `rate * volume` was non-finite: a non-finite or negative `rate`, or a non-finite `lo`/`hi`, reached `std::poisson_distribution`, whose behavior is undefined for a non-finite mean. It now validates that `rate` is finite and non-negative and that every `lo`/`hi` is finite (and guards against `volume` overflowing to infinity), raising a clean `ValueError`. Likewise `random.noisy_sin` / `random.noisy_cos` segfaulted on `n_points=0` — each PCF needs at least one breakpoint — and now raise `ValueError`. ([#31](https://github.com/kthtda/stablebear/issues/31), [#32](https://github.com/kthtda/stablebear/issues/32), [#33](https://github.com/kthtda/stablebear/issues/33))
