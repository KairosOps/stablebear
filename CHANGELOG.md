...
* **`split(t, 0)` raises instead of crashing the interpreter** — `split` with `n_sections=0` performed an integer modulo-by-zero (`axis_size % n_sections`) and aborted the process with SIGFPE. It now raises a clean `ValueError("split: number of sections must be > 0")`, matching the guard `array_split` already had. ([#19](https://github.com/kthtda/stablebear/issues/19))
