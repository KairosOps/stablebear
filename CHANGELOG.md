...
* **`bool()` on a numeric tensor reflects the value, not the length** — `FloatTensor`/`IntTensor` had no `__bool__`, so `bool()` fell back to `__len__` and returned the first-dimension length: every non-empty numeric tensor was truthy (`bool(IntTensor([0]))` was `True`), multi-element tensors silently failed to raise, and a 0-d tensor raised `len() of unsized object`. `__bool__` now lives on the shared `Tensor` base (replacing the `BoolTensor`-only copy), so a single-element tensor — including a 0-d scalar — returns its element's truthiness and any other size raises `ValueError`, matching NumPy. ([#45](https://github.com/kthtda/stablebear/issues/45))
