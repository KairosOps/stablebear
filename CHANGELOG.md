...
* **Rank-0 (scalar) tensors are now computed instead of silently zeroed** — constructing a numeric or bool tensor from a 0-d NumPy scalar (`FloatTensor(np.array(3.5))`) dropped the value and returned the dtype's zero, and arithmetic whose broadcast result was rank-0 (`zeros(()) + 1.0`, a `squeeze()`-ed size-1 tensor, a `reshape([])` result, …) returned `0` while in-place ops were silent no-ops. The element walk skipped any tensor whose shape was empty, conflating a rank-0 scalar (which has exactly one element) with a zero-size extent (no elements). It now visits the single rank-0 element, so 0-d construction preserves the scalar and 0-d arithmetic matches NumPy (`np.array(5.0) + 1.0 == 6.0`); genuinely empty tensors (a dimension of size 0) are still skipped. ([#3](https://github.com/kthtda/stablebear/issues/3), [#8](https://github.com/kthtda/stablebear/issues/8))
