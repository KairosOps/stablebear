...
* **Assigning an array into a `PointCloudTensor` slice distributes one cloud per cell** — `t[:] = arr`, `t[...] = arr`, and partial-index `t[i] = arr` on a `PointCloudTensor` stored the *entire* array as a single cloud broadcast into every selected cell, silently corrupting the result. An ndarray right-hand side is now distributed across the selected cells (its leading axes index cells, its trailing `(n_points, dim)` axes form each cloud, like `PointCloudTensor(arr)`), and a leading shape that does not match the selected region raises `ValueError`. Full-index assignment of a single cloud (`t[i] = cloud`) is unchanged. ([#40](https://github.com/kthtda/stablebear/issues/40))
