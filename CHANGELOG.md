...
* **`==` / `!=` against a scalar return a `BoolTensor`** — comparing a numeric tensor to a Python or NumPy scalar (`t == 2.0`) returned a plain Python `bool` via Python's identity fallback, silently breaking the documented mask-select idiom `t[t == 2.0]` (the index was a scalar `bool`, so it selected nothing) and `< <= > >=` against a scalar were the only working comparisons. `==` and `!=` now broadcast a scalar or `ndarray` right-hand side and return an elementwise `BoolTensor`, consistent with the ordering operators ([#57](https://github.com/kthtda/stablebear/issues/57)) and NumPy. ([#47](https://github.com/kthtda/stablebear/issues/47))
