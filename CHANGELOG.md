...
* **A broadcast view is now read-only, matching NumPy** — `broadcast_to` returned a writable stride-0 view, so item assignment through it (`view[0, 0] = x`) silently scattered the write across the broadcast axis and corrupted the shared source, and in-place arithmetic (`view += 1`) double-wrote each physical cell. As in `numpy.broadcast_to`, writing through a broadcast view (item assignment or any in-place operator) now raises `ValueError`; the view remains fully readable, and `.copy()` yields a writeable tensor. Size-1 axes inserted by `newaxis`/`expand_dims` are unaffected (no aliasing). ([#16](https://github.com/kthtda/stablebear/issues/16), [#9](https://github.com/kthtda/stablebear/issues/9))
