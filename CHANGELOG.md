...
* **Ragged nested lists are rejected instead of silently misplacing elements** — building a tensor from a nested list (e.g. `PcfTensor([...])`) validated only the *first* sub-list at each depth, so ragged input whose total element count happened to equal a rectangular product (rows of length 2, 3, 1 → 6 = 3×2) was silently accepted and the elements scattered into the wrong `(i, j)` positions. Every branch is now validated against the inferred shape, so any ragged structure (including a non-sequence sibling where a sub-list is expected) raises `ValueError("Ragged nested list: …")`, matching the documented "rectangular" contract. ([#4](https://github.com/kthtda/stablebear/issues/4))
