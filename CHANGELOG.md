...
* **Self-aliasing slice assignment no longer corrupts data** — assigning an overlapping view of a tensor into itself (`a[:] = a[::-1]`, `a[1:] = a[:-1]`) silently produced wrong results, because the element-wise copy in `assign_from` read positions it had already overwritten. When the right-hand side shares storage with the destination, the RHS is now materialized into a temporary first (matching NumPy's overlap handling), so e.g. `a[:] = a[::-1]` reverses correctly. Non-overlapping and cross-dtype assignments are unaffected (no extra copy). ([#6](https://github.com/kthtda/stablebear/issues/6))
* **Self-aliasing slice assignment of an object-element tensor keeps cells independent** — extending the overlapping-assignment fix to point-cloud, `DistanceMatrix`, and `SymmetricMatrix` tensors: `t[1:] = t[:-1]` (or `t[:] = t[::-1]`) duplicated only the outer buffer, so the matrix/cloud elements stayed shared with the source cells. The overlap copy now deep-copies each stored element too, so every cell is independent. ([#129](https://github.com/kthtda/stablebear/issues/129))
* **Storing a matrix or point cloud in a tensor cell now copies it (no aliasing)** — assigning a `DistanceMatrix`, `SymmetricMatrix`, or point cloud into a tensor cell (`t[i] = m`) or a slice (`t[0:2] = src`) shared the source's underlying buffer instead of copying it: these element types hold a `std::shared_ptr` and were copied shallowly. Mutating the source afterwards, or assigning one object into several cells, then silently corrupted the stored cells (and the source). Element storage now deep-copies the element (via a `copy()` used only at the cell-store boundary, leaving the internal shallow-sharing that `pdist`/`l2_kernel` rely on untouched), so cells are independent of the source and of each other, matching NumPy object-array assignment. ([#39](https://github.com/kthtda/stablebear/issues/39))
