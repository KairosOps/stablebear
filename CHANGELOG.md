...
* **`Pcf` rejects negative breakpoint times (the `t=0` invariant is checked after sorting)** — `Pcf` construction validated the `t=0` requirement on the *unsorted* input row 0, then sorted the breakpoints, so a negative time on a later row was moved to the front and produced a malformed PCF with `t_0 < 0` that returned values at negative times instead of raising. The minimum time is now validated after sorting, so any input whose smallest time is not 0 (in particular any negative time) raises `ValueError`; breakpoints given out of order with a valid `t=0` are accepted. Evaluation also guards against `t < 0` directly rather than against the first breakpoint's time. ([#11](https://github.com/kthtda/stablebear/issues/11))
